...
void pwc_reset(void)
{ 
  crm_periph_reset(CRM_PWC_PERIPH_RESET, AT_TRUE);
  crm_periph_reset(CRM_PWC_PERIPH_RESET, AT_FALSE);
}
...
/**
  ...
  *         this parameter can be one of the following values:
  *         - AT_TRUE <wakeup pin is used for wake up cpu from standby mode>
  *         - AT_FALSE <wakeup pin is used for general purpose I/O>
  * @retval none
  */
...
{   
  if(pwc_flag == PWC_STANDBY_FLAG)
    PWC->ctrl_bit.clsef = AT_TRUE;
  if(pwc_flag == PWC_WAKEUP_FLAG)
    PWC->ctrl_bit.clswef = AT_TRUE;
}  
...
{ 
  PWC->ctrl_bit.clswef = AT_FALSE;
  PWC->ctrl_bit.lpsel = AT_TRUE;  
  SCB->SCR |= 0x04;
#if defined (__CC_ARM)
  __force_stores();
#endif   
