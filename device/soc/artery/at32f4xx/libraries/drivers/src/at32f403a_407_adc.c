...
void adc_reset(adc_type *adc_x)
{
  if(adc_x == ADC1)
  {
    crm_periph_reset(CRM_ADC1_PERIPH_RESET, AT_TRUE);
    crm_periph_reset(CRM_ADC1_PERIPH_RESET, AT_FALSE);
  }
  else if(adc_x == ADC2)
  {
    crm_periph_reset(CRM_ADC2_PERIPH_RESET, AT_TRUE);
    crm_periph_reset(CRM_ADC2_PERIPH_RESET, AT_FALSE);
  }
  else if(adc_x == ADC3)
  {
    crm_periph_reset(CRM_ADC3_PERIPH_RESET, AT_TRUE);
    crm_periph_reset(CRM_ADC3_PERIPH_RESET, AT_FALSE);
  }
}
...
/**
  ...
  *         this parameter can be: AT_TRUE or AT_FALSE.
...
{
  if(new_state == AT_TRUE)
  {
    adc_x->ctrl1 |= adc_int;
  }
  else if(new_state == AT_FALSE)
  {
    adc_x->ctrl1 &= ~adc_int;
  } 
}
...
{
  adc_x->ctrl2_bit.adcalinit = AT_TRUE;
}
...
{
  adc_x->ctrl1_bit.ocvmen = AT_FALSE;
  adc_x->ctrl1_bit.pcvmen = AT_FALSE;
  adc_x->ctrl1_bit.vmsgen = AT_FALSE;
