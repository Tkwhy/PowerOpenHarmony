...
void i2c_reset(i2c_type *i2c_x)
{
  if(i2c_x == I2C1)
  {
    crm_periph_reset(CRM_I2C1_PERIPH_RESET, AT_TRUE);
    crm_periph_reset(CRM_I2C1_PERIPH_RESET, AT_FALSE);
  }
  else if(i2c_x == I2C2)
  {
    crm_periph_reset(CRM_I2C2_PERIPH_RESET, AT_TRUE);
    crm_periph_reset(CRM_I2C2_PERIPH_RESET, AT_FALSE);
  }
  else if(i2c_x == I2C3)
  {
    crm_periph_reset(CRM_I2C3_PERIPH_RESET, AT_TRUE);
    crm_periph_reset(CRM_I2C3_PERIPH_RESET, AT_FALSE);
  }
}
...
/**
  ...
  *         this parameter can be one of the following values:  
  *         I2C1, I2C2, I2C3.
  * @param  new_state (AT_TRUE or AT_FALSE)
  * @retval none
  */
...
void i2c_init(i2c_type *i2c_x, i2c_fsmode_duty_cycle_type duty, uint32_t speed)
{
  ...
  i2c_x->ctrl1_bit.i2cen = AT_FALSE;
  ...
  if(speed <= 100000)
  {
    ...
    /* disable fast mode */
    i2c_x->clkctrl_bit.speedmode = AT_FALSE;
  ...
  else
  {
    ...
    i2c_x->clkctrl_bit.speedmode = AT_TRUE;
...
{
  i2c_x->ctrl1_bit.genstart = AT_TRUE;
}
...
{
  i2c_x->ctrl1_bit.genstop = AT_TRUE;
}
