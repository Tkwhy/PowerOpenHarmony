...
int LittlefsRead(int partition, UINT32 *offset, void *buf, UINT32 size)
{
    ...
    stmflash_read(startAddr, buf, size/ sizeof(uint32_t));
...
int LittlefsProg(int partition, UINT32 *offset, const void *buf, UINT32 size)
{
    ...
    stmflash_write(startAddr, buf, size/ sizeof(uint32_t));
...
int LittlefsErase(int partition, UINT32 offset, UINT32 size)
{
    ...
    stmflash_erase(startAddr, size/ sizeof(uint32_t));
...
int LittlefsRead(int partition, UINT32 *offset, void *buf, UINT32 size)
{
    W25x_BufferRead((uint8_t*)buf, *offset, size);
    return LFS_ERR_OK;
}

int LittlefsProg(int partition, UINT32 *offset, const void *buf, UINT32 size)
{
    W25x_BufferWrite((uint8_t*)buf, *offset, size);
    return LFS_ERR_OK;
}
